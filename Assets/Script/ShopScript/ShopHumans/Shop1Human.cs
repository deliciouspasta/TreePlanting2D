...
public class Shop1Human : MonoBehaviour {
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面でヒトの画像を表示する
			ShopStatic.shopHuman[1].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在のヒトの日誌番号
			ShopStatic.shopNowDiaryHumanNumber = 1;

			//日誌を表示、背景タブを表示
			ShopStatic.shopHumanExplainTab[1].SetActive(true);
