...
public class Shop4Human : MonoBehaviour {
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面でヒトの画像を表示する
			ShopStatic.shopHuman[4].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在のヒトの日誌番号
			ShopStatic.shopNowDiaryHumanNumber = 4;

			//日誌を表示、背景タブを表示
			ShopStatic.shopHumanExplainTab[4].SetActive(true);
