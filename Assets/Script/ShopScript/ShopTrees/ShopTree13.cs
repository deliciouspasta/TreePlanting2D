...
public class ShopTree13 : MonoBehaviour {
    ...
    //この木の１００万本の値段
	long thisTreePrice = 0;
	long thisTreePriceRainbow = 1000;
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面で木の画像を表示する
			ShopStatic.shopTree[13].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在の木の日誌番号
			ShopStatic.shopNowDiaryTreeNumber = 13;

			//日誌を表示、背景タブを表示
			ShopStatic.shopTreeExplainTab[13].SetActive(true);
