...
public class ShopTree12 : MonoBehaviour {
    ...
    //この木の１００万本の値段
	long thisTreePrice = 2700000000;
	long thisTreePriceRainbow = 227;
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面で木の画像を表示する
			ShopStatic.shopTree[12].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在の木の日誌番号
			ShopStatic.shopNowDiaryTreeNumber = 12;

			//日誌を表示、背景タブを表示
			ShopStatic.shopTreeExplainTab[12].SetActive(true);
