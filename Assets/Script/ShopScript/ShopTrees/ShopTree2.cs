...
public class ShopTree2 : MonoBehaviour {
	...
	public void OnClicking(){
		...
		if(isPressUp){
			//確認画面で木の画像を表示する
			ShopStatic.shopTree[2].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在の木の日誌番号
			ShopStatic.shopNowDiaryTreeNumber = 2;

			//日誌を表示、背景タブを表示
			ShopStatic.shopTreeExplainTab[2].SetActive(true);
