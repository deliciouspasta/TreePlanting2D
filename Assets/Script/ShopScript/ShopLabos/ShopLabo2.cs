...
public class ShopLabo2 : MonoBehaviour {
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面で研究所の画像を表示する
			ShopStatic.shopLabo[2].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在の研究所の日誌番号
			ShopStatic.shopNowDiaryLaboNumber = 2;

			//日誌（便宜）を表示、背景タブを表示
			ShopStatic.shopLaboExplainTab[2].SetActive(true);
