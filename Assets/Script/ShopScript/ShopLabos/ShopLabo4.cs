...
public class ShopLabo4 : MonoBehaviour {
    ...
    //この研究所の買い取り値段
	long thisLaboPriceRainbow = 3000;
	...
	public void OnClicking(){
		...
		if(isPressUp == true){
			...
			//確認画面で研究所の画像を表示する
			ShopStatic.shopLabo[4].SetActive(true);
	...
	public IEnumerator TimeForPointerDown(){
		...
		if(isPressDown){
			...
			//現在の研究所の日誌番号
			ShopStatic.shopNowDiaryLaboNumber = 4;

			//日誌（便宜）を表示、背景タブを表示
			ShopStatic.shopLaboExplainTab[4].SetActive(true);
